...
public class DeletePersonajeDialog extends JDialog {
    ...
    public DeletePersonajeDialog(JFrame parent, Runnable onSuccess) {
    	super(parent, "Eliminar Personaje", true);
    ...
    private void eliminarPersonaje() {
        ...
        boolean confirm = DialogUtils.confirm(
                this,
                "¿Seguro que quieres eliminar el personaje con ID " + id + "?",
                "Confirmar eliminación"
        );

        if (!confirm) return;
        
        boolean eliminado = ApiClient.eliminarPersonaje(id);

        if (eliminado) {
            DialogUtils.showInfo(this, "Personaje eliminado correctamente");
